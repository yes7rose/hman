...
SERVER_PORT = None
print __file__+"\\serverport.txt"
with open(os.path.dirname(__file__)+"\\serverport.txt", 'r') as infoPort:
    SERVER_PORT = int(infoPort.readline().replace("\n",""))
...
parser.add_option("-p", "--port", action="store", dest="port", type="int", 
    metavar="PORT", default=SERVER_PORT, help="specify a different "
...
parser.add_option("-q", "--quiet", action="store_true", dest="quiet", 
    default=True, help="quiet mode (no logging). in stdio mode, "
    "writes to /dev/null")
...
def setPort(parser, port):
    parser.add_option("-p", "--port", action="store", dest="port", type="int", 
        metavar="PORT", default=port, help="specify a different "
        "TCP listener port. Default is 18812")
